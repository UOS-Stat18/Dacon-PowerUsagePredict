...
class LTSF_NLinear(torch.nn.Module):
    ...
    def forward(self, x_mean, x_res):
        ...
        x = seasonal_output / 2 + trend_output * 2 #분해되었던 추세(이동평균)와 계절(잔차)을 다시 더해줌
        x = torch.clamp(x, min = 0, max = 4)
        x = x.permute(0,2,1)
...
for i in range(1,101):
    ...
    with tqdm(range(epoch)) as tr:
        for j in tr:
            ...
            if valid_loss<=best_valid:
                torch.save(model, 'C:\\dacon\\Dacon-PowerUsagePredict\\js_park\\saved_models\\best_AF_3.pt')
                best_valid=loss
    ...
    model = torch.load('C:\\dacon\\Dacon-PowerUsagePredict\\js_park\\saved_models\\best_AF_3.pt')
...
wpcnf.to_csv(r'C:\dacon\Dacon-PowerUsagePredict\js_park\submit\test_3.csv', index = False)