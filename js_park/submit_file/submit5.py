...
class LTSF_NLinear(torch.nn.Module):
    ...
    def forward(self, x_mean, x_res):
        ...
        x = torch.clamp(x, min = 0.01, max = 1)
        x = x.permute(0,2,1)
...
for i in range(66,67):
    ...
    with tqdm(range(epoch)) as tr:
        for j in tr:
            ...
            if valid_loss<=best_valid:
                torch.save(model, 'C:\\dacon\\Dacon-PowerUsagePredict\\js_park\\saved_models\\best_AF_6.pt')
                best_valid=loss
    ...
    model = torch.load('C:\\dacon\\Dacon-PowerUsagePredict\\js_park\\saved_models\\best_AF_6.pt')
...
wpcnf.to_csv(r'C:\dacon\Dacon-PowerUsagePredict\js_park\submit\test_6.csv', index = False)

len(smape_list)
np.mean(smape_list) #7.827440900960937
