...
for i in range(74,101):
    ...
    with tqdm(range(epoch)) as tr:
        for j in tr:
            ...
            if valid_loss<=best_valid:
                torch.save(model, 'C:\\dacon\\Dacon-PowerUsagePredict\\js_park\\saved_models\\best_AF_4.pt')
                best_valid=loss
    ...
    model = torch.load('C:\\dacon\\Dacon-PowerUsagePredict\\js_park\\saved_models\\best_AF_4.pt')
...
wpcnf.to_csv(r'C:\dacon\Dacon-PowerUsagePredict\js_park\submit\test_4.csv', index = False)

len(smape_list)
np.mean(smape_list) # 7.044057177240029