...
train_valid[['기온(C)','강수량(mm)','풍속(m/s)','습도(%)','연면적(m2)','냉방면적(m2)']] = min_max_scaler.transform(train_valid[['기온(C)','강수량(mm)','풍속(m/s)','습도(%)','연면적(m2)','냉방면적(m2)']])
...
with tqdm(range(epoch)) as tr:
    for i in tr:
        ...
        if valid_loss<=best_valid:
            torch.save(model, r'C:\dacon\Dacon-PowerUsagePredict\js_park\best_AF_michin.pth')
            patient=0
            best_valid=loss
        else:
            patient+=1
...
model = torch.load(r'C:\dacon\Dacon-PowerUsagePredict\js_park\best_AF_michin.pth')
...
test_x[['기온(C)','강수량(mm)','풍속(m/s)','습도(%)','연면적(m2)','냉방면적(m2)']] = min_max_scaler.fit_transform(test_x[['기온(C)','강수량(mm)','풍속(m/s)','습도(%)','연면적(m2)','냉방면적(m2)']])
