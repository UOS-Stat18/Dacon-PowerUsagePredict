...
class LTSF_NLinear(torch.nn.Module):
    def __init__(self, input_size, temp_size,output_size, target_size):
        ...
        self.Linear_Trend1 = torch.nn.Linear(self.input_size, self.temp_size )
        self.Linear_Seasonal1 = torch.nn.Linear(self.input_size,  self.temp_size )
        self.Linear_Trend2 = torch.nn.Linear(self.temp_size , self.output_size)
        self.Linear_Seasonal2 = torch.nn.Linear(self.temp_size ,  self.output_size)
    ...
    def forward(self, x_mean, x_res):
        ...
        trend_output = self.Linear_Trend1(trend_output)
        trend_output = self.relu(trend_output)
        trend_output = self.Linear_Trend2(trend_output).permute(0,2,1)

        seasonal_output = self.Linear_Seasonal1(seasonal_output)
        seasonal_output = self.relu(seasonal_output)
        seasonal_output = self.Linear_Seasonal2(seasonal_output).permute(0,2,1)
...
with tqdm(range(epoch)) as tr:
    for i in tr:
        ...
        if valid_loss<=best_valid:
            torch.save(model, r'C:\dacon\Dacon-PowerUsagePredict\js_park\best_AF_seed2.pth')
            patient=0
            best_valid=loss
        else:
            patient+=1
...
model = torch.load(r'C:\dacon\Dacon-PowerUsagePredict\js_park\best_AF_seed2.pth')
