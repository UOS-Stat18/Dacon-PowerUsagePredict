...
class LTSF_NLinear(torch.nn.Module):
    ...
    def forward(self, x_mean, x_res):
        ...
        trend_output = torch.clamp(trend_output, min = -0.5, max = 0.5)
        seasonal_output = torch.clamp(seasonal_output, min = -0.4, max = 0.4)
        x = seasonal_output + trend_output #분해되었던 추세(이동평균)와 계절(잔차)을 다시 더해줌
        x = torch.clamp(x, min = 0.1, max = 1.1)
        x = x.permute(0,2,1)
...
for i in range(1,101):
    ...
    with tqdm(range(epoch)) as tr:
        for j in tr:
            ...
            if valid_loss<=best_valid:
                torch.save(model, 'C:\\dacon\\Dacon-PowerUsagePredict\\js_park\\saved_models\\best_AF_5.pt')
                best_valid=loss
    ...
    model = torch.load('C:\\dacon\\Dacon-PowerUsagePredict\\js_park\\saved_models\\best_AF_5.pt')
...
wpcnf.to_csv(r'C:\dacon\Dacon-PowerUsagePredict\js_park\submit\test_5.csv', index = False)

len(smape_list)
np.mean(smape_list) #8.02112716017291